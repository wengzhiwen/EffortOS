...
### M3: 用户设定 `[x]` — Sprint 3 + Sprint 8 ✓
> 心率区/功率区配置 + 用户认证

- [x] 用户注册/登录（邮箱验证码，参考 MotionO 模式）
- [x] 心率区间配置 CRUD（Z1-Z5 阈值设定）
- [x] 功率区间配置 CRUD
- [x] FTP / 最大心率 / LTHR 等关键参数设置
- [x] 个人资料页面
...
### M7: 用户认证 `[x]` — Sprint 7 ✓
> 参考 MotionO 模式（邮箱验证码登录）

- [x] 用户注册/登录（邮箱验证码）
- [x] Session 管理（Bearer token + httponly cookie）
- [x] API 层级数据隔离（写操作需认证，读操作按用户过滤）
- [ ] 用户资料页面
...
### M10: 竞品差异化功能 `[~]`
> Sprint 36-38 新增，参考竞品研究

- [x] Best Efforts 最佳表现（Strava）
- [x] 训练趋势图（周 TSS + 训练天数）
- [x] 强度等级可视化和筛选
- [x] 装备里程追踪（TrainingPeaks）
- [x] 活动分段分析（Strava）
- [x] Wellness 日常追踪（intervals.icu）
- [x] Readiness 准备度分数（intervals.icu）
- [x] 活动对比功能（Strava）
